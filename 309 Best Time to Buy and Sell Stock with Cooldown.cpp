...
int calprof(int total,int i, vector<int>s,int flag,unordered_map<string,int>& mp){
    

    // Base condition
    if(i==(s.size()-1)){
        if(flag==0)//better not to buy
            return total;
        else if(flag==1){//cool down
            return total;
        }
        else if(flag==2){//ofcourse sell
            return total+s[i];
        }
    }
    
    //Checking whether we have it already in our database
    //preprocessing temp
    string temp_ans=to_string(i);
    temp_ans.append(" ");
    temp_ans.append(to_string(total));
    temp_ans.append(" ");
    temp_ans.append(to_string(flag));
    
    if(mp.find(temp_ans)!=mp.end())
        return mp[temp_ans];
    ...
    if(flag==0){//can buy
        ...
        temp=to_string(i+1);
        temp.append(" ");
        temp.append(to_string(total-s[i]));
        temp.append(" ");
        ...
        if(mp.find(temp)!=mp.end())
            ...
            first=calprof(total-s[i],i+1,s,2,mp); 
            mp[temp]=first;
        
        temp=to_string(i+1);
        temp.append(" ");
        temp.append(to_string(total));
        temp.append(" ");
        ...
        if(mp.find(temp)!=mp.end())
            ...
            second=calprof(total,i+1,s,flag,mp);
            mp[temp]=second;
        
        // first=calprof(total-s[i],i+1,s,2,mp);
        // second=calprof(total,i+1,s,flag,mp);
        return mp[temp_ans]=max(first,second);  //mp[temp_ans]=max(mp[temp1],mp[temp2]);
        // return max(first,second);

        
    }
    else if(flag==2){//can sell
        ...
        if(mp.find(temp)!=mp.end())
            ...
            first=calprof(total+s[i],i+1,s,1,mp);
            mp[temp]=first;
        ...
        // first=calprof(total+s[i],i+1,s,1,mp);
        // second=calprof(total,i+1,s,flag,mp);
        return mp[temp_ans]=max(first,second);
        // return max(first,second);
        ...
        if(mp.find(temp)!=mp.end())
            return mp[temp_ans]=mp[temp];
        else
            return mp[temp_ans]=calprof(total,i+1,s,0,mp);  
                
        // return calprof(total,i+1,s,0,mp); 
    }
...
public:
    /*
    // Memorization - DP Even applicable k cooldowns
    vector<int> mem;
    int findMax(vector<int>& prices,int curr,int n)
    {
        ...
        for(int i=curr+1;i<n;++i)
            if(prices[curr]<prices[i])  //We can try to sell on ith day
                ...
                maxVal = max(maxVal, prices[i]-prices[curr]+findMax(prices,i+2,n)); // for k cooldowns replace i+2-> i+k+1
    ...
    */
    ...
    int maxProfit(vector<int>& prices) {
        ...
        return calprof(total,0,prices,0,mp);//start by buying stocks
        */
        ...
        */
        ...
        // Below part works only for k=1 cooldown

        // more optimized O(n) solution is available using state transition but that might be
        // applicable only for 1 countdown may be
        // Best time to buy and sell ,with k days of cooldown
        // Check Youtube link below
        //https://youtu.be/4wNXkhAky3s
        ...
        */
        ...
        int noStock = 0;
        int inHand = -prices[0];    //bcoz we have bought a stock and never sold it
        int Sold = 0;
        int temp_noStock=noStock;
        int temp_inHand=inHand;
        int temp_Sold=Sold;
        
        for(int i=1;i<n;++i)
        {
            temp_noStock = max(noStock,Sold);
            temp_inHand = max(inHand,noStock-prices[i]);
            temp_Sold = inHand+prices[i];
            noStock=temp_noStock;
            inHand=temp_inHand;
            Sold=temp_Sold;
        }
        
        int result = max(noStock,Sold);
        return result;
